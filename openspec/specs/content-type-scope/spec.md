# content-type-scope Specification

## Purpose
TBD - created by archiving change content-type-scope. Update Purpose after archive.
## Requirements
...
choose a content-type scope (Text only / Images only / Text & Images),
apply the chosen scope to the current active note, and persist the choice
as the new last-used scope for future "Start watch mode" invocations. The
three option labels SHALL be shown in the active locale; the persisted
scope value (`"text"` / `"image"` / `"both"`) is unaffected by locale.
...
- **THEN** the content-type dropdown shows the Spanish translations of
  "Text only", "Images only", and "Text & Images"
